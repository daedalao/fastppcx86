...
| amongthesleep | Unity / Mono | 2026-08-04 | **runs on fixed build (caged)** — no wedge. Quality bugs: green artifacts in title video (video decode or GL-thunk path), audio stutter (known audio class), and erratic player MODEL behavior — skinning/animation math is dense packed-SSE + scalar FP, so model weirdness is a candidate for a further VectorOps semantics bug (NaN/DAZ/rounding), same neighborhood as today's CR0 fix but a different failure mode | user live run 2026-08-04 13:2x |
