...
| "the problems were with **Mono**" | inherited framing, relayed second-hand | **IMPRECISE, AND THE IMPRECISION COST US.** The original report was about **monojit** — Mono's JIT specifically, not the runtime broadly. "Mono" reads as a runtime-integration question and sent us through packaging and filesystem archaeology; "monojit" reads as a codegen/decode question and points directly at defect 1c, which is what it turned out to be — FEX faulting while decoding blocks monojit had just emitted into its arenas. A one-word difference in an inherited claim selected the wrong investigation for most of a day. Worth remembering that verifying an inherited claim includes verifying you have it *stated* correctly |
| the right Mono test workload is compiling C# | our own rung-1 choice | **WRONG WORKLOAD.** `mcs` is a C# *compiler*; no real target invokes it, since shipped managed games ship precompiled assemblies. Running a precompiled `.exe` is both smaller *and* representative, and exercises the same JIT/SMC/signal machinery. Chosen for convenience rather than derived from the goal |
...
| 1c | ~~**Mono JIT arenas fault during decode**~~ | **FIXED** — `f05a2b949` | `cf17977e8` added an unconditional 16-byte read of `_InstStream` at the top of `DecodeInstructionsAtEntry` — compile hot path, every block, every host — with three comments asserting faults were tolerated and no handler anywhere. Any guest instruction beginning within 16 bytes of a mapped-region end faulted during decode. Mono guarantees that geometry: 19 RWX arenas via `MAP_FIXED_NOREPLACE`, 64 KiB payload plus a deliberate 4 KiB guard. A guest RIP of `0x7feddff2`, 14 bytes below its arena end, produced `SEGV_MAPERR` at exactly `0x7fede000` — byte 14 of the read. Now opt-in and page-clamped. **Verified:** `mcs` compiles, `hello.exe` runs and prints, 9264 signals all `SEGV_ACCERR` with zero `MAPERR` and zero SIGILL, ctest 6/7011 on both paths. **CORRECTION — not an upstream bug.** I twice called this guest-arch and therefore an upstream candidate. Wrong. The diagnostic block exists in exactly two commits: `cf17977e8` (daedalao, 2026-05-17) which added it, and `f05a2b949` which fixed it. The last upstream-FEX merge in this history is 2026-04-28, three weeks earlier — so it was added to this fork only and upstream never had it. That is why it has never surfaced in the games and platforms FEX runs elsewhere. Still worth telling the fork author, since Ziggurat's Mono could hit the same arena geometry, but it is a fork-local regression rather than a contribution |
| 1d | `c1ac6dac6` dispatcher `li r0, 0` | **Defensive, possibly unobservable** | Cherry-picked from main. Reverting it produces **no** symptom in the tiered ladder, the threaded tier, or `probe_r0_index` — while reverting `cf608d750` reliably SIGSEGVs tier 5. `r0` is also re-zeroed by `FillForABICall` and `FillStaticRegs`, so this may guard a case that never occurs. Kept, correctly; coverage claim deliberately **not** made. Not worth further investigation — no user-facing consequence either way |
