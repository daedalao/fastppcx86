...
| `FEX_SMCGRANULEMIXED` | 64K hosts only. The mixed code/data granule heuristic: faults per granule per second at which a granule holding at most `FEX_SMCGRANULEMIXEDMAXTRACKED` tracked pages is **demoted** -- mtrack stops write-protecting it for the rest of its life and every block compiled from any of its guest pages carries the per-instruction `ValidateCode` guard that `SMCCHECKS=full` uses (same soundness, paid only on those pages). The demoting fault always invalidates the whole granule; the soft relink refuses retained blocks on a demoted page; the code cache rejects a section touching one. Default `64`, `0` disables (forced off on a 4K host). Logged as `demoted (#n)` through the flip-log line. |
