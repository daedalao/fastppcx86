...
DEF_OP(Div) {
  ...
  if (IROp->Size <= IR::OpSize::i32Bit) {
    // Build a 64-bit signed dividend in TMP1.
    if (LongDiv) {
      auto Upper = GetReg(Op->Upper);
      // Dividend = (Upper << 32) | (Lower & 0xFFFFFFFF). Mask Lower because it
      // may be sign-extended in the SRA register.
      rldicl(TMP4, Lower, 0, 32);
      sldi(TMP1, Upper, 32);
      or_(TMP1, TMP1, TMP4);
    } else {
      extsw(TMP1, Lower);  // sign-extend the 32-bit dividend to 64
    }
    extsw(TMP4, Divisor);
    divd(Quotient, TMP1, TMP4);
    mulld(TMP3, Quotient, TMP4);
    subf(Remainder, TMP3, TMP1);
    // Results are 32-bit; mask high halves so SRA writebacks are clean.
...
DEF_OP(UDiv) {
  ...
  if (IROp->Size <= IR::OpSize::i32Bit) {
    if (LongDiv) {
      ...
      rldicl(TMP1, Lower, 0, 32);
    }
    rldicl(TMP4, Divisor, 0, 32);
...
DEF_OP(Ror) {
  ...
  if (IsInlineConstant(Op->Src2, &Const)) {
    ...
    if (IROp->Size <= IR::OpSize::i32Bit) {
      rlwinm(Dst, S1, 32 - (rot & 31), 0, 31);
    } else {
      // rotate right = rotate left by (64 - rot)
      rldicl(Dst, S1, 64 - rot, 0);
    }
  } else {
    if (IROp->Size <= IR::OpSize::i32Bit) {
...
DEF_OP(Adc) {
  ...
  auto Dst = GetReg(Node);
  adde(Dst, GetReg(Op->Src1), GetReg(Op->Src2));
}
...
DEF_OP(AdcWithFlags) {
  ...
  if (IROp->Size == IR::OpSize::i64Bit) {
    // 64-bit ADC: addeo. sets CA + OV + CR0 from the 64-bit boundary, which
    // is what x86 wants for 64-bit ADC.
    addeo_(Dst, S1, S2);
    return;
  }

  // i32Bit: do a 64-bit add-with-carry on zero-extended operands. Bit 32 of
  // the result = x86 CF; XER.CA from the 64-bit boundary is always 0 here so
  // we patch CA explicitly. OF derived from sign-bit rule at 32-bit width.
  rldicl(TMP1, S1, 0, 32);                // S1[31:0]
  rldicl(TMP2, S2, 0, 32);                // S2[31:0]
  adde(TMP3, TMP1, TMP2);                  // TMP3 = TMP1 + TMP2 + CA_in (≤ 33-bit)
  rldicl(Dst, TMP3, 0, 32);                // Dst = result low-32, zero-ext
  EmitTestNZSetCR(Dst, IR::OpSize::i32Bit);

  // CF = bit-32 of TMP3, in LSB.
  rldicl(TMP4, TMP3, 32, 63);

  // OF = (S1[31] == S2[31]) AND (S1[31] != Dst[31]).
  rldicl(TMP1, S1,  33, 63);               // S1[31]
  rldicl(TMP2, S2,  33, 63);               // S2[31]
  rldicl(TMP3, Dst, 33, 63);               // Dst[31]
  xor_(TMP2, TMP2, TMP1);                  // S1^S2
  xor_(TMP3, TMP3, TMP1);                  // S1^Dst
  andc(TMP3, TMP3, TMP2);                  // (S1^Dst) AND NOT(S1^S2) = OF
...
DEF_OP(AdcZero) {
  ...
  // Add carry flag to source
  addze(GetReg(Node), GetReg(Op->Src1));
}

DEF_OP(AdcZeroWithFlags) {
  auto Op = IROp->C<IR::IROp_AdcZeroWithFlags>();
  auto Dst = GetReg(Node);
  addze(Dst, GetReg(Op->Src1));
  if (IROp->Size == IR::OpSize::i32Bit) {
    rldicl(Dst, Dst, 0, 32);
    EmitTestNZSetCR(Dst, IR::OpSize::i32Bit);
  } else if (IROp->Size == IR::OpSize::i64Bit) {
    cmpdi(Dst, 0);
  } else {
    EmitTestNZSetCR(Dst, IROp->Size);
  }
}
...
DEF_OP(AdcNZCV) {
  ...
  rldicl(TMP1, S1, 0, 32);
  rldicl(TMP2, S2, 0, 32);
  adde(TMP3, TMP1, TMP2);                  // TMP3 = sum + CA_in (33-bit, full)
  rldicl(TMP4, TMP3, 0, 32);                // TMP4 = low-32 result
  EmitTestNZSetCR(TMP4, IR::OpSize::i32Bit);

  // CF = bit-32 of TMP3 (full pre-mask), in LSB.
  rldicl(TMP3, TMP3, 32, 63);

  // OF = (S1[31] == S2[31]) AND (S1[31] != Result[31]).
  rldicl(TMP1, S1,   33, 63);              // S1[31]
  rldicl(TMP2, S2,   33, 63);              // S2[31]
  rldicl(TMP4, TMP4, 33, 63);              // Result[31] (TMP4 still holds masked result, bit 31 of low32)
  xor_(TMP2, TMP2, TMP1);                  // S1^S2
  xor_(TMP4, TMP4, TMP1);                  // S1^Result
  andc(TMP4, TMP4, TMP2);                  // (S1^Result) AND NOT(S1^S2) = OF

  // Patch XER: CA = TMP3 LSB, OV = TMP4 LSB.
  mfspr(TMP1, 1);
  LoadConstant(TMP2, 0x60000000ULL);
  andc(TMP1, TMP1, TMP2);
  sldi(TMP2, TMP3, 29);
...
DEF_OP(Select) {
  ...
  if (Dst != False) mr(Dst, False);    // default: false value
  PPC64Emitter::Label Done{};
  bc(InvertCond(CC), &Done);   // if condition NOT taken, Dst = False (already set)
  mr(Dst, True);               // condition taken: overwrite with True
  Bind(&Done);
}
...
DEF_OP(MaskGenerateFromBitWidth) {
  ...
  // Generate a mask of the lower BitWidth bits (all ones in [0, BitWidth-1])
  // Result = (1 << BitWidth) - 1
  li(TMP1, 1);
  sld(TMP1, TMP1, BitWidth);
  addi(Dst, TMP1, -1);
}
...
DEF_OP(CondSubNZCV) {
  ...
  if (IROp->Size == IR::OpSize::i32Bit) {
    ...
    if (S2Inline) {
      int64_t NegC = -static_cast<int64_t>(Const);
      if (NegC >= -32768 && NegC <= 32767) {
        addic_(TMP3, S1, static_cast<int16_t>(NegC));
        // Clear OV (addic. doesn't compute it).
        mfspr(TMP1, 1);
        LoadConstant(TMP2, 0x40000000ULL);
        andc(TMP1, TMP1, TMP2);
        mtspr(1, TMP1);
...
DEF_OP(ShiftFlags) {
  ...
  // OF (defined for count=1; we always compute):
  //   LSL: OF = MSB(Src1) ^ MSB(Result) (sign-bit change)
  //   LSR: OF = MSB(Src1)
  //   ASR: OF = 0
  ...
  } else if (Op->Shift == IR::ShiftType::LSR) {
    srdi(OFReg, Src1, SzBits - 1);
    rldicl(OFReg, OFReg, 0, 63);
  } else { // ASR
    li(OFReg, 0);
  }
...
DEF_OP(CPUID) {
  ...
  // and at +272..+360 AFTER SpillForABICall):
  //   [r1+ 0]  back chain
  //   [r1+ 8]  CPUIDObj  (this)
  //   [r1+16]  LR save
  ...
  // Marshal args into ABI registers (post-spill mini-frame is at +272).
  ld(r3,     8 + 272, r1);  // r3  = this (CPUIDObj)
  ld(r4,    32 + 272, r1);  // r4  = Function
  ld(r5,    40 + 272, r1);  // r5  = Leaf
  ld(r(12), 24 + 272, r1);  // r12 = callee (ELFv2 indirect-call req)

  std(r2, 48 + 272, r1);    // save TOC across C call
  mtctr(r(12));
  bctrl();
  ld(r2, 48 + 272, r1);     // restore TOC
  // Function returns 16-byte aggregate in r3:r4.

  // Save result to mini-frame slots (post-spill: +272+56, +272+64).
  std(r3, 56 + 272, r1);
  std(r4, 64 + 272, r1);
...
DEF_OP(XGetBV) {
  ...
  ld(r3,     8 + 272, r1);  // r3  = CPUIDObj
  ld(r4,    32 + 272, r1);  // r4  = Function
  ld(r(12), 24 + 272, r1);  // r12 = XCRFunction
...
DEF_OP(RDRAND) {
  ...
  std(r3, 24 + 272, r1);  // save 64-bit result
...
DEF_OP(MonoBackpatcherWrite) {
  ...
  // Marshal args (post-spill +272).
  mr(r3, STATE);                          // arg0: Frame*
  li(r4, IR::OpSizeToSize(Op->Size));     // arg1: Size (uint8_t, zero-extended)
  ld(r5,    24 + 272, r1);                // arg2: Addr
  ld(r6,    32 + 272, r1);                // arg3: Value
  ld(r(12),  8 + 272, r1);                // r12 = callee
...
DEF_OP(LoadDF) {
  // Load direction flag from CpuStateFrame
  auto Dst = GetReg(Node);
