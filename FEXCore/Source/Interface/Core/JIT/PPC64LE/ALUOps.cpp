...
DEF_OP(NZCVSelect) {
  ...
  // Branch-free select via isel (ISA 2.03). This is the guest CMOVcc / SETcc
  // archetype: the condition is genuine data (flags of a prior compare), which
  // is exactly the case where a branch mispredicts and isel wins. MapNZCVCC
  // above has already projected XER→CR1 / composed CR3 bits as needed, and
  // returns CC.BI as an absolute CR bit index — pass it straight to isel.
  if (is_const_true) {
    ...
    // False side is zero: place it in whichever isel slot the condition sense
    // demands. iselcc puts vfalse in the RA slot for BO==4 conditions (where
    // RA=0 encodes LITERAL zero) and in the RB slot for BO==12 (where r0
    // reads as 0 via the backend's r0≡0 invariant). Same values as the old
    // li 0 / bc / li ±1 sequence, including the 64-bit -1 for the all_ones
    // form at i32Bit (upper bits ignored per the 32-bit invariant).
    li(TMP1, const_true == all_ones ? -1 : 1);
    iselcc(Dst, CC, TMP1, r0);
    return;
  }
