...
// is bypassed because (a) it wires Control to r3 for VPCMPISTRX while the
// fallback C signature places Control in r7 (after two slot-skipping vector
// args), and (b) any non-leaf bctrl through the dispatcher's PushDynamicRegs
// area corrupts the r26 (RA[2]) spill slot when the helper saves LR to
// caller_r1+16. The Op_Unhandled path snapshots that slot before the call
// and restores it afterwards. No DEF_OP bodies are needed here.
...
DEF_OP(VAESImc) {
  ...
  // Snapshot r26 spill slot before the helper clobbers it (see EMIT_AES_ROUND).
  ld(TMP1, 16, r1);
  std(TMP1, PostSpill(CryptoSlotR26), r1);
  ...
  // Restore r26 spill slot before PopDynamicRegs.
  ld(r(0), PostSpill(CryptoSlotR26), r1);
  std(r(0), 16, r1);
...
#define EMIT_AES_ROUND(HelperFn, StateReg, KeyReg, DstReg)                  \
  do {                                                                       \
    ...
    SpillForABICall(TMP1);                                                   \
    /* Snapshot post-spill [r1+16] (where PushDynamicRegs parked r26) before */\
    /* the helper's prologue clobbers it with its incoming LR.               */\
    ld(TMP1, 16, r1);                                                         \
    std(TMP1, PostSpill(CryptoSlotR26), r1);                                  \
    addi(r3, r1, PostSpill(CryptoSlotA));                                    \
    addi(r4, r1, PostSpill(CryptoSlotA));                                    \
