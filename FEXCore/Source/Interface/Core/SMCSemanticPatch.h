...
//
// (e) Not handled, on purpose.  Only rel32 direct branches are recognised.
//     mov-immediate patches are NOT supported: the JIT may fold, narrow or
//     rematerialise a guest immediate anywhere in the block, and there is no
//     fixed-width relocation window to find it in (InsertGuestRIPMove exists
//     only for guest RIPs).  Recognising those would require a new per-constant
//     relocation kind; patchloop-style JITs patch call/jmp targets, so branch-
//     only is the right MVP.  rel8 branches, indirect branches, and rel16 are
//     also out (the size check rejects them).
//
...
/**
 * @brief Test one ExitRIPSite against an old/new target pair.
 *
 * On SiteMatch::Patchable, *Out is appended-to by the caller; NeedsWrite is
 * false when the two RIPs encode identically (nothing to do).
 */
